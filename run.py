...
class Run():
    def __init__(self,
        ...
        if config['dataset'] == "pheme":
            ...
            self.train_path = self.root_path + 'train_data.xlsx'
            self.val_path = self.root_path + 'val_data.xlsx'
            self.test_path = self.root_path + 'test_data.xlsx'
            self.category_dict = {
                "经济": 0,
                "健康": 1,
                "军事": 2,
    ...
    def main(self):
        ...
        if self.model_name == 'MDAN':
            trainer = Trainer(emb_dim=self.emb_dim, mlp_dims=self.mlp_dims, bert=self.bert,
