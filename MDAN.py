...
from .MMDFND import MultiDomainPLEFENDModel
from .layers import *
from .pivot import *
...
class MDAN(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, bert, out_channels, dropout, num_experts, num_heads):
        super(MDAN, self).__init__()
        self.num_expert = num_experts  # 初始情况为6
        ...
        for i in range(self.domain_num):
            image_gate = nn.Sequential(nn.Linear(self.unified_dim * 3, self.unified_dim),
                                       nn.SiLU(),
                                       # SimpleGate(),
            ...
            text_gate = nn.Sequential(nn.Linear(self.unified_dim * 3, self.unified_dim),
                                      nn.SiLU(),
                                      # SimpleGate(),
                                      # nn.BatchNorm1d(int(self.unified_dim / 2)),
                                      nn.Linear(self.unified_dim, self.num_expert * 3),
            ...
            fusion_gate = nn.Sequential(nn.Linear(self.unified_dim * 4, self.unified_dim),
                                        nn.SiLU(),
                                        # SimpleGate(),
                                        # nn.BatchNorm1d(int(self.unified_dim / 2)),
            ...
            final_gate = nn.Sequential(nn.Linear(1088 + 320, 720),
                                       nn.SiLU(),
                                       # SimpleGate(),
                                       # nn.BatchNorm1d(int(self.unified_dim / 2)),
                                       nn.Linear(720, 160),
        ...
        self.mapping_CC_MLP_mu = nn.Sequential(
            nn.Linear(1, self.unified_dim),
            nn.SiLU(),
            # nn.BatchNorm1d(self.unified_dim),S
            nn.Linear(self.unified_dim, 1),
        )
        ...
        hyperparam = {}
        hiddensize = 1280
        hyperparam["n_hidden"] = 32
        hyperparam['fusion_dim'] = [hiddensize, hiddensize, hiddensize]
        hyperparam['n_head'] = self.num_heads  # 初始值为4
        hyperparam['n_layer'] = 8
        hyperparam['fusion_dropout'] = 0.6

        hyperparam['num_classes'] = 2
        hyperparam['modal_num'] = 3
        self.mul_fusion = VLTransformer_Gate(hyperparam['fusion_dim'], hyperparam, out_dim=320)

    def fusion_img_text(self, image_emb, text_emb, fusion_emb, mlp_img, mlp_text, mlp_fusion):
        ...
        batchsize = img_feature_seq.shape[0]
        img_feature_seq = img_feature_seq.view(batchsize, -1)
        text_feature_seq = text_feature_seq.view(batchsize, -1)
        fusion_feature_seq = fusion_feature_seq.view(batchsize, -1)

        fusion = torch.concat([img_feature_seq, text_feature_seq, fusion_feature_seq], dim=-1)

        item_emb_trans = self.mul_fusion(fusion)
    ...
    def forward(self, **kwargs):
        ...
        text_gate_input = torch.cat([domain_embedding, text_atn_feature, text_feature[:, -1, :].squeeze(1)],
                                    dim=-1)  # ([64, 1536])
        image_gate_input = torch.cat([domain_embedding, image_atn_feature, image_feature[:, -1, :].squeeze(1)], dim=-1)
        fusion_gate_input = torch.cat([domain_embedding, fusion_atn_feature, fusion_feature[:, -1, :].squeeze(1)],
                                      dim=-1)
        ...
        cross_knowledge = self.fusion_img_text(image_gate_share_expert_value, text_gate_share_expert_value,
                                               fusion_gate_share_expert_value, self.mlp_img, self.mlp_text,
                                               self.pivot_mlp_fusion)
        domain_special_list = []
        for i in range(self.domain_num):
            ...
            domain_knowledge = self.fusion_img_text(image_spacial_knowledge, text_spacial_knowledge,
                                                    fusion_spacial_knowledge, self.mlp_img_list[i],
                                                    self.mlp_text_list[i], self.pivot_mlp_fusion_list[i])
        ...
        for i in range(self.domain_num):
            ...
            share_sigma.append(
                self.mapping_T_MLP_sigma(torch.sigmoid(share_score_list[i]).clone().detach().view(-1, 1)))
        ...
        for i in range(self.domain_num):
            ...
            final_dom_feature0 = F.normalize(domain_special_list[i], dim=-1)
            final_share_feature0 = F.normalize(domain_special_list[i], dim=-1)
        ...
        for i in range(self.domain_num):
            ...
            final_gate_input = torch.cat([domain_embedding, fusion_tempfeat_main_task,
                                          (concat_feature_list[i][:, 0, :] + concat_feature_list[i][:, 1, :]) / 2],
                                         dim=-1)
...
class Trainer():
    ...
    def train(self):
        self.model = MDAN(self.emb_dim, self.mlp_dims, self.bert, 320, self.dropout,
                                              self.num_experts, num_heads=self.num_heads)
        if self.use_cuda:
        ...
        for epoch in range(self.epoches):
            ...
            for step_n, batch in enumerate(train_data_iter):
                ...
                final_label_pred_list, fusion_label_pred_list, image_label_pred_list, text_label_pred_list = self.model(
                    **batch_data)
                ...
                loss = (1 - self.alpha) * loss0 + self.alpha * (loss1 + loss2 + loss3)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
